# GX10 문서

GX10 Code/Vision Brain 구축 관련 문서입니다.

---

## 핵심 문서

| 문서 | 설명 |
|------|------|
| [implementation-guide.md](implementation-guide.md) | 최종 구현 가이드 |
| [two-brain-architecture.md](two-brain-architecture.md) | Two Brain 아키텍처 및 최적화 |
| [build-checklist.md](build-checklist.md) | 구축 체크리스트 |
| [external-access.md](external-access.md) | 외부 접근 가이드 (Tailscale) |

---

## 스크립트 문서

| 문서 | 설명 |
|------|------|
| [../scripts/install/README.md](../scripts/install/README.md) | 설치 스크립트 상세 |

---

## 오류 해결 (Knowledge Base)

| 문서 | 설명 |
|------|------|
| [../memory/errors/](../memory/errors/) | KB-001 ~ KB-014 오류 및 해결책 |

---

## 아카이브

초기 계획 및 히스토리 문서:

| 문서 | 설명 |
|------|------|
| [archive/GX10-00-install-guide.md](archive/GX10-00-install-guide.md) | 초기 설치 가이드 |
| [archive/GX10-01-Setup-Plan.md](archive/GX10-01-Setup-Plan.md) | 초기 설정 계획 |
| [archive/GX10-02-Setup-Supplement-v1.1.md](archive/GX10-02-Setup-Supplement-v1.1.md) | 설정 보충 |
| [archive/GX10-05-Execution-Plan.md](archive/GX10-05-Execution-Plan.md) | 실행 계획 |
| [archive/GX10-06-Comprehensive-Guide.md](archive/GX10-06-Comprehensive-Guide.md) | 종합 가이드 |
| [archive/GX10-07-Auto-Installation-Plan.md](archive/GX10-07-Auto-Installation-Plan.md) | 자동 설치 계획 |
| [archive/GX10-08-CodeBrain-Memory-Optimization.md](archive/GX10-08-CodeBrain-Memory-Optimization.md) | Code Brain 메모리 최적화 |

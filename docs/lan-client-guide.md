...
| 모델 | 용도 | 속도 |
|------|------|------|
| `gpt-oss:120b` | 범용 (서치·코딩·RAG 추론) | **38.5 tok/s** (Ollama v0.30.6 실측) |
| `qwen3-embedding:latest` | 텍스트 임베딩 (벡터 생성) | — |

> **단일 엔드포인트 원칙:** 모든 추론 요청은 `gpt-oss:120b` 하나로 처리됩니다.  
> 호출 시스템에서 모델을 선택하거나 구분할 필요가 없습니다.  
> `gpt-oss:120b`는 116.8B MXFP4 MoE 모델, 131K 컨텍스트, 메모리 점유 ~76 GiB.  
> **주의 (KB-019):** `/api/generate` + `num_predict < 1000` 조합 시 빈 응답 반환 (thinking 모델 특성).  
> → `/api/chat` 엔드포인트 사용 또는 `num_predict ≥ 1000` 설정 필수.
...
| 최대 컨텍스트 | 모델 기본값 | gpt-oss:120b: 131K |
...
| 상황 | 예상 속도 |
|------|-----------|
| 일반 질의 | **38.5 tok/s** (Ollama v0.30.6 실측) |
| 장문 컨텍스트 (RAG) | 25–35 tok/s (컨텍스트 길이에 따라 감소) |

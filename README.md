...
| 가동 시간 | 1일 7:44 |
| CPU 부하 (1/5/15분) | 0.24 / 0.10 / 0.08 |
| 메모리 | 56 GiB 사용 / 119 GiB (43.7%) |
| 디스크 (`/`) | 158 GB 사용 / 983 GB (16.1%) |
| GPU | NVIDIA GB10, 44°C, 10.54W |
| GPU 드라이버 | 580.159.03 |
| CUDA | 13.0 |
| GPU 메모리 사용 | 31,611 + 14,386 MiB ≈ **46 GiB** |
| 활성 Brain | Code Brain (2026-02-14 이후 유지) |
| Ollama | v0.23.2 실행 중 |
...
| 모델 | GPU 메모리 | 컨텍스트 | 상태 |
|------|-----------|---------|------|
| `kqwen-coder:latest` | 31,611 MiB | 32,768 | ✅ GPU 활성 |
| `qwen3-embedding:latest` | 14,386 MiB | 40,960 | ✅ GPU 활성 |

> 권장 상시 구성: kqwen-coder + qwen3-embedding = ~46 GiB (GB10 실효 한도 이내)

### 서비스 상태

| 서비스 | 포트 | 상태 | 비고 |
|--------|------|------|------|
| Ollama | 11434 | ✅ 실행 중 | systemd 관리 |
| Open WebUI | 8080 | ✅ healthy | Docker, 2시간 가동 |
| SearXNG | 8888 | ✅ 실행 중 | Docker, 32시간 가동 |
| Dashboard | 9000 | ✅ 실행 중 | systemd 관리 |
...
```bash
# 기반 모델 업데이트
for m in qwen3.6:latest devstral-small-2:latest gemma4:26b qwen3-embedding:latest; do
  ollama pull $m
done

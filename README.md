...
| GPU | NVIDIA GB10 (SM 12.1, cuda_v13) |
| GPU 드라이버 | 580.159.03 |
| CUDA | 13.0 |
| Ollama | **v0.30.6** 실행 중 (2026-06-08 업그레이드) |
| 디스크 (`/`) | 161 GB 사용 / 916 GB |

### 운영 모델 (단일 엔드포인트)

| 모델 | 크기 | 용도 | 실측 속도 |
|------|------|------|----------|
| `gpt-oss:120b` | 65 GB | 범용 추론 (코딩·서치·RAG) | **38.5 tok/s** |
| `qwen3-embedding:latest` | 4.7 GB | 텍스트 임베딩 (4096차원) | — |

> **단일 엔드포인트 원칙**: 모든 추론 요청은 `gpt-oss:120b` 하나로 처리. 클라이언트에서 모델 선택 불필요.
> gpt-oss:120b는 116.8B MXFP4 MoE 모델, 131K 컨텍스트, 메모리 점유 ~76 GiB.
> Ollama v0.30.6 NVIDIA 성능 개선으로 21.6 tok/s(v0.23.2) → 38.5 tok/s(v0.30.6) 향상.

### 서비스 상태

| 서비스 | 포트 | 상태 | 비고 |
|--------|------|------|------|
| Ollama | 11434 | ✅ 실행 중 | systemd 관리, v0.30.6 |
| Open WebUI | 8080 | ✅ healthy | Docker |
| SearXNG | 8888 | ✅ 실행 중 | Docker |
...
| 계정 | sudo 권한 있는 사용자 |
| 로컬 연결 (선택) | t3610과 2.5G 직결 — `enx00e04c4728ca` (192.168.100.1/24) |
...
# 네트워크 접속 (LAN)
http://10.20.6.141:9000

# t3610에서 2.5G 직결 접속 (최우선)
http://192.168.100.1:9000

# Tailscale 접속 (외부)
http://100.78.1.7:9000
```
...
| 메모리 | 128GB LPDDR5x Unified Memory (실측 가용: 119 GiB — GPU/CPU 통합 단일 풀) |
...
| CUDA | 13.0 |
| Ollama | v0.30.6 |
| LAN (공용망) | 1G Ethernet — `enP7s7` (10.20.6.141) |
| LAN (t3610 직결) | 2.5G Ethernet — `enx00e04c4728ca` (192.168.100.1/24), 실측 2.35Gbps |
...
### AI 모델 운영 현황 (2026-06-08 기준)

| 항목 | 상태 |
|------|------|
| 범용 추론 (`gpt-oss:120b`, MXFP4 MoE 116.8B) | ✅ 운영 중 — 38.5 tok/s |
| RAG 임베딩 (`qwen3-embedding`, 4096차원, 7.6B Q4_K_M) | ✅ 운영 중 |
| Ollama v0.30.6 업그레이드 | ✅ 완료 (2026-06-08) |

> **메모리 현황 (GB10 실측)**: 119 GiB 통합 메모리.
> gpt-oss:120b 점유 ~76 GiB + qwen3-embedding ~5 GiB = 합계 ~81 GiB.
> 나머지 ~38 GiB는 OS/KV cache용 여유.

> **주의 — qwen3-coder-next(80B) GB10 버그**: llama.cpp #23010 (OPEN)
> aarch64 × GB10 조합에서 전체 `?` 토큰 출력 버그. 설치 금지.
> qwen3-coder:30b(MoE 30B/3.3B, 19GB)는 버그 없음 — 추가 검토 가능.
...
- [Qwen3 공식](https://github.com/QwenLM/Qwen3)
